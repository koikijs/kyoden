...
class Timeline extends Component {
  ...
  render() {
    ...
    const width =
      moment(this.props.end).diff(moment(this.props.start), 'days') * this.props.widthOfDate;
    const timeWidth = (this.props.widthOfDate / DATE_OF_MINUTES) * this.props.ticksMinutes;
    return (
      <div className={this.props.styles.timeline.container}>
        <ul
        ...
        >
          <li className={this.props.styles.timeline.currentDate}>
            {moment(this.props.start)
              .add(this.state.displayed, 'days')
        ...
        >
          ...
          >
            {this.props.items.map(item => (
              <li key={item.id} className={this.props.styles.timeline.line}>
                <ul className={this.props.styles.timeline.periods}>
                  {item.periods.map(period => (
                    <li
                      key={period.id}
                      className={`${this.props.styles.timeline.period} ${
                        period.className ? period.className : ''
                      }`}
                      style={{
                        left:
                          moment(period.start).diff(moment(this.props.start), 'seconds') *
                          (this.props.widthOfDate / DATE_OF_SECONDS),
                        width:
                          moment(period.end).diff(moment(period.start), 'seconds') *
                          (this.props.widthOfDate / DATE_OF_SECONDS),
                      }}
                    >
